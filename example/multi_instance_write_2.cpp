...
int main(int argc, char* argv[])
{
    ...
    // PDMlibのセットアップ(シングルトン版)
    {
        PDMlib::PDMlib::GetInstance().Init(argc, argv, "PDMlibWriteSample_2.dfi");
        PDMlib::PDMlib::GetInstance().SetBaseFileName("PDMlibWriteSample_2");
    ...
    for(int TimeStep = 0; TimeStep < 1000; TimeStep++)
    {
        ...
        if(TimeStep%100 == 0)
        {
            ...
            //後半のデータをシングルトン版を使って出力
            PDMlib::PDMlib::GetInstance().Write("ParticleID",  NumData/2, ID+NumData/2, (int*)NULL, 1, TimeStep, Time);
            PDMlib::PDMlib::GetInstance().Write("Coordinate",  NumData/2, Coordinate+NumData/2, (float*)NULL, 3, TimeStep, Time);
            PDMlib::PDMlib::GetInstance().Write("Velocity",    NumData/2, Velocity+NumData/2, vMinMax2, 3, TimeStep, Time);
            PDMlib::PDMlib::GetInstance().Write("Temparature", NumData/2, Temparature+NumData/2, tMinMax2, 1, TimeStep, Time);
        }
    }
