...
namespace PDMlib
{
  ...
  {
    public:
      MetaData(std::string arg_filename) : Version("0.2"),
      Comm(MPI_COMM_WORLD),
      Communicator("MPI_COMM_WORLD"),
      ReadOnly(false),
      HeaderOutput(false),
      DirectoryPath("pdm"),
      FieldFilenameFormat("rank_step"),
    ...
    public:
      ...
      //! フィールドデータのベースファイル名を設定する
      void SetBaseFileName(const std::string filename){if(!ReadOnly)this->Prefix = filename;}

      //! フィールドデータのベースファイル名を取得する
      std::string GetBaseFileName(void) const {return Prefix;}

      //! フィールドデータ名のフォーマットを設定する
      void SetFileNameFormat(const std::string& format)
      {
        std::string format_lower(format); // transformに渡す前にformatと同じサイズで領域を確保させるためにコピーコンストラクタを呼んでいる
        std::transform(format.begin(), format.end(), format_lower.begin(), ::tolower);
        if (format_lower == "step_rank")
        {
      ...
      //! フィールドデータを格納するディレクトリ名を取得する
      //
      //Pathが絶対パスで指定されている時はそのまま返し、相対パスで指定されている時は
      //DFIファイルが存在するパスからの相対パスを返す
      std::string GetPath(void) const
      {
        const char sep = '/';
        std::string actual_path = "";
        if (DirectoryPath[0] != sep)
        {
          std::string::size_type n = FileName.find_last_of(sep);
          if (n != std::string::npos)
          {
            actual_path = FileName.substr(0, n+1);
          }
        }
        actual_path += DirectoryPath;
        return actual_path;
      }
    ...
    private:
      ...
      //! フィールドデータのベースファイル名
      //
      //! 実際に使われるファイル名は "Prefix_Rank番号_タイムステップ.拡張子”となり
      //! 拡張子はContainerInfo構造体の中でコンテナ毎に指定されたものが使われる
      std::string Prefix;

      //! ファイル名を生成する時にPrefixの後にRank番号_タイムステップの順で付与するか
      //! タイムステップ_Rank番号の順で付与するかを決める
      std::string FieldFilenameFormat;

      //! フィールドデータを格納するディレクトリ名
